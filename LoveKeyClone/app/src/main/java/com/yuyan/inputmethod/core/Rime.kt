    ...
    companion object {
        ...
        init {
            try {
                ...
                android.util.Log.e("Rime", "Failed to load libyuyanime.so", e)
            }
        }

        fun startup(context: Context, fullCheck: Boolean) {
            startupRime(context, context.filesDir.absolutePath + "/rime", context.filesDir.absolutePath + "/rime", fullCheck)
            updateStatus()
        }
